...
题目：[qilinglab-release](https://www.shielder.it/blog/2021/07/qilinglab-release/)
...
def hook_uname_on_exit(ql: Qiling, buf, *args, **kwargs):
    ...
    # DI、EDI、RDI都是目的变址寄存器
    # ida 能看到结构体的信息和每个变量的大小
    ql.mem.write(buf, b'QilingOS\x00')
    ql.mem.write(buf + 65 * 3, b'ChallengeStart\x00')
    # 有人写入数据的时候这样子 b'ChallengeStart'.ljust(65, b'\x00') 填充0, 不填充也能过 
    # #todo 默认补齐 0 了吗
    return 0
...
def c7(ql: Qiling):
    ...
    # 或者 hook syscall sleep 调用的是 nanosleep 系统调用
...
- https://zhuanlan.zhihu.com/p/408383879
